...
class Agent:
    ...
    def get_state(self):
        ...
        state.append(1) if 90 < self.get_obs(self.obs.angle_to_dropzone) < 270 and self.velocity_z > 0 else state.append(0)
        ...
        if action_list[0] == 1:
            if action_list[1] == 1:
                if self.get_obs(observation_index) is not None:
                    angleToDebris = self.get_obs(observation_index)
