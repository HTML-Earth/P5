...
class Agent:
    ...
    def __init__(self):
        ...
        # Last distance
        self.last_distance_to_debris = 0
    ...
    def get_state(self):
        ...
        state.append(1) if self.get_obs(self.obs.robot_facing_debris) and self.velocity_z > 0 else state.append(0)
        ...
        # Distance to debris
        if int(self.get_obs(self.obs.getting_closer_to_debris_1)) > 20:
            state.append(self.last_distance_to_debris)
        else:
    ...
    def distance_to_debris(self, state, action, observation_index):
        ...
        # Temporary fix to overwhelmingly large number
        if distance > 20:
            return self.last_distance_to_debris

        self.last_distance_to_debris = distance
    ...
    def getting_closer_to_dropzone(self, state, action):
        ...
        if action_list[0] == 1:
            if action_list[1] == 0:
                if 90 < self.get_obs(self.obs.angle_to_dropzone) < 270:
                    return 1
                else:
                    return 0
        ...
        if action_list[0] == -1:
            if action_list[1] == 0:
                if (90 > self.get_obs(self.obs.angle_to_dropzone) > 0) or (270 < self.get_obs(self.obs.angle_to_dropzone) < 360):
                    return 1
                else:
                    return 0
